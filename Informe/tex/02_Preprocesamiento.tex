...
\begin{itemize}
    ...
    \item \textbf{name}: Esta columna fue eliminada por ser un identificador sin valor predictivo directo.
    \item \textbf{host\_id}: Análogo a \texttt{name}. 
...
\begin{table}[H]
    ...
    \begin{tabular}{lc}
        ...
        \texttt{last\_review}        & Genera \texttt{days\_since\_last\_review} y \texttt{never\_reviewed} \\
        \texttt{name}                & Eliminación \\
...
\input{./tex/tables/df_raw_train_desc.tex}

La Tabla \ref{tab:df-raw-train-desc} resume los estadísticos descriptivos antes de aplicar el recorte con IQR.
...
Una versión sin aplicar IQR se incluye en el apéndice (Tabla \ref{tab:df-preprocessed-train-desc}).
